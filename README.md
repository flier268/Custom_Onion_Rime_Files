...
> **本倉庫為洋蔥 Rime 方案的個人自訂版本**
> 官方原版：[oniondelta/Onion_Rime_Files](https://github.com/oniondelta/Onion_Rime_Files)

### 主要差異

本自訂版本針對個人使用習慣進行了以下修改：

#### 1. 按鍵配置調整

修改了五個主要輸入方案（純注音、plus、mixin、雙拼、行列）的按鍵行為：

| 按鍵 | 原始行為 | 自訂行為 | 說明 |
|------|----------|----------|------|
| **左 Shift** | 切換中英文模式 | 切換到臨時英文模式（inline_ascii） | 輸入英文後自動回到中文模式 |
| **右 Shift** | 切換中英文模式 | 輸出注音字母（commit_code） | 將注音符號轉為對應的英文字母（如：ㄅ → b） |
| **CapsLock** | 切換中英文模式 | 禁用（noop） | 避免誤觸切換模式 |

#### 2. Backspace 刪除行為

- **預設行為**：改為逐字母刪除（原版為刪除整組拼音）
- **Shift + Backspace**：刪除整組字符
- **優點**：提供更精確的編輯控制

#### 3. 其他優化

- 清理與 IDE 衝突的快捷鍵配置
- 添加自動同步與發布工作流程（GitHub Actions）

### 適用對象

此自訂版本適合：
- 習慣使用左/右 Shift 區分功能的使用者
- 需要頻繁輸入注音字母（如教學、標註）的使用者
- 偏好精確編輯控制（逐字母刪除）的使用者
- 不使用 CapsLock 切換輸入法的使用者

### 還原至官方設定

如需使用官方原始設定，請參考：
- [官方 Wiki](https://github.com/oniondelta/Onion_Rime_Files/wiki)
- 或直接從官方倉庫下載原版配置檔
